...
def home_admin():
    ...
    if search_form.validate_on_submit() and search_form.search.data:
        search_user = User.query.filter((User.surname == search_form.surname.data) | (User.email == search_form.email.data) | (User.session_token == search_form.session_token.data)).all()
        return render_template('admin/home_admin.html', users = users, search_list = search_user, search_form = search_form)

    return render_template('admin/home_admin.html', users = users, search_form = search_form)
...
def patients_searched():
    ...
    patients = Patient.query.with_entities(Patient.name, Patient.surname, Patient.dob, Patient.pid).filter((Patient.pid == find) | (Patient.surname == find)).all()
...
def patient_gen():
    if StoreID.id == 0:
        ...
        return render_template('/gen_pdf/template.html', patient = patient, obj = patient.photo, image = image, time = time)
